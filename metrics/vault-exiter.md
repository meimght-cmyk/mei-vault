# Vault-exiter

_Updated: 2026-07-08T12:07:46.793Z_

Polling guardian for Phase 3. Watches every harness-confirmed position and polls `/api/score` at ~60s cadence. On a degradation transition (ALLOW→WARN/BLOCK, decision→ERROR, or +3000 bps risk jump), it emits an exit event with a would-be-tx payload. **No signing, no broadcast** — Phase 4 swaps the boolean for a real bounded-delegation withdraw.

## Current state

- open positions: **5**
- positions exited: **260**
- total exit events logged: **260**

## Trigger rules

| trigger | condition |
|---|---|
| `allow_to_warn` | entry decision was ALLOW, current is WARN |
| `allow_to_block` | entry decision was ALLOW, current is BLOCK |
| `became_error` | current decision is ERROR (pool unreachable / state corrupt) |
| `risk_jump` | live riskBps − entry riskBps ≥ 3000 |

## Recent exit events (last 30)

| ts | intent | protocol | entry → current | drift | trigger |
|---|---|---|---|---|---|
| 2026-07-08 07:30:54 | `spot-swap-base-2026-07-07-001` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-08 07:30:53 | `passive-lp-kumbaya-2026-07-07-001` | kumbaya | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-08 07:30:51 | `passive-lp-kumbaya-2026-07-07-003` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-08 07:30:51 | `spot-swap-base-2026-07-08-003` | uniswap-v3-base | ALLOW/500 → ERROR/-1 | -501 | became_error |
| 2026-07-08 07:30:49 | `spot-swap-base-2026-07-08-001` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-08 07:30:47 | `passive-lp-kumbaya-2026-07-08-003` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-08 07:00:08 | `spot-swap-base-2026-07-07-003` | uniswap-v3-base | ALLOW/500 → ERROR/-1 | -501 | became_error |
| 2026-07-07 22:20:59 | `spot-swap-base-2026-07-07-002` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-07 16:19:06 | `spot-swap-base-2026-07-05-001` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-07 06:38:12 | `passive-lp-kumbaya-2026-07-05-002` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-07 06:38:12 | `spot-swap-base-2026-07-06-001` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-07 06:38:11 | `spot-swap-base-2026-07-06-003` | uniswap-v3-base | ALLOW/500 → ERROR/-1 | -501 | became_error |
| 2026-07-06 06:03:07 | `spot-swap-base-2026-07-05-002` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-06 06:03:06 | `passive-lp-kumbaya-2026-07-05-001` | kumbaya | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-06 06:03:05 | `passive-lp-kumbaya-2026-07-05-003` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-06 06:03:04 | `passive-lp-kumbaya-2026-07-06-003` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-06 06:03:03 | `passive-lp-kumbaya-2026-07-06-002` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-06 06:03:03 | `passive-lp-kumbaya-2026-07-06-001` | kumbaya | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-06 05:37:30 | `spot-swap-base-2026-07-05-003` | uniswap-v3-base | ALLOW/500 → ERROR/-1 | -501 | became_error |
| 2026-07-06 05:27:15 | `spot-swap-base-2026-07-04-002` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-05 09:18:34 | `passive-lp-kumbaya-2026-07-04-003` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-05 05:09:46 | `passive-lp-kumbaya-2026-07-04-001` | kumbaya | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-05 05:09:46 | `spot-swap-base-2026-07-04-001` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-05 05:09:44 | `spot-swap-base-2026-07-04-003` | uniswap-v3-base | ALLOW/500 → ERROR/-1 | -501 | became_error |
| 2026-07-05 04:18:34 | `passive-lp-kumbaya-2026-07-04-002` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |
| 2026-07-01 11:35:58 | `spot-swap-base-2026-06-30-003` | uniswap-v3-base | ALLOW/500 → ERROR/-1 | -501 | became_error |
| 2026-07-01 11:35:42 | `spot-swap-base-2026-06-30-002` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-01 11:35:27 | `spot-swap-base-2026-06-30-001` | uniswap-v3-base | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-01 11:35:11 | `passive-lp-kumbaya-2026-06-30-001` | kumbaya | ALLOW/0 → ERROR/-1 | -1 | became_error |
| 2026-07-01 11:34:56 | `passive-lp-kumbaya-2026-06-30-003` | kumbaya | ALLOW/2000 → ERROR/-1 | -2001 | became_error |

## How to read this

- An exit event means: "if the vault were holding this position, the guardian would now route a withdraw to the signer."
- Multiple triggers can fire on the same position over time, but once a position is marked `EXITED` it's removed from the watch list.
- Raw events: [`ledger/exit-events.jsonl`](../ledger/exit-events.jsonl). State cache: [`ledger/exit-state.json`](../ledger/exit-state.json).

## What "signer-less" means

✓ load positions from harness-confirmed intents
✓ poll `/api/score` for each
✓ detect degradation transitions
✓ build the would-be-withdraw payload
✗ construct real `decreaseLiquidity` calldata (Phase 4: positionManager ABI + tick math)
✗ sign with a wallet (Phase 4: bounded-delegation signer)
✗ broadcast to chain (Phase 4: `/api/preflight-raw` + `eth_sendRawTransaction`)
