...
10. **Aerodrome v2 (Solidly) shipped 2026-05-23.** Package `packages/claw-protocols/src/aerodrome-v2-base/` — different math from Slipstream (no slot0/ticks, uses `getReserves()` + immutable `stable()` flag). 27,636 pools live on Base via factory `0x420DD381b…`. Scoring includes a custom Solidly-stable depeg signal (reserves deviating from decimal-normalized 1:1). Tested on USDC/WETH volatile pool `0xcDAC0d6c…` (riskBps=0) and USDC/USDT stable pool `0x96508AE8…` (riskBps=2000, caught USDT's `mint(address,uint256)` selector — admin can inflate supply). Aggregator version `v0.7`.

11. **Moonwell (Compound v2 fork lending) shipped 2026-05-23.** Package `packages/claw-protocols/src/moonwell-base/`. Reads mToken state (`getCash`, `totalBorrows`, `totalReserves`, supply/borrow rates per timestamp, exchange rate, underlying, reserve factor). Scoring channels: utilization (>95% = exit-stall risk, >98% = critical), borrow APR (>500% = runaway curve), inactive market, underlying token-pattern checks, reserve-factor sanity. Repurposes `oracleHealthBps` to encode utilization headroom (10000-utilization_bps) so downstream consumers can compare across protocols. Tested on mUSDC via direct invocation: utilization 89.67%, riskBps=0, `"elevated but workable"`. Known mTokens table covers mUSDC/mWETH/mcbETH/mDAI. Aggregator version `v0.8`. Reads are batched in groups of 4 to stay under public Base RPC concurrency limits.
