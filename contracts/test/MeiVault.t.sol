...
contract ReentrantExecTarget {
    ...
    function attack() external {
        vault.exec(address(this), abi.encodeWithSelector(this.attack.selector));
    }
}
...
contract MeiVaultTest is Test {
    ...
    function test_constructor_revertsOnBadStaleness() public {
        vm.expectRevert(abi.encodeWithSelector(MeiVault.StalenessOutOfRange.selector, 599));
        new MeiVault(
            IERC20(address(token)), "x", "x", owner, feeRecipient, CAP, MIN, DELAY, 599, 50, 500
        );
    }

    function test_constructorRevertsStalenessTooLow() public {
        vm.expectRevert(abi.encodeWithSelector(MeiVault.StalenessOutOfRange.selector, 599));
        new MeiVault(
            IERC20(address(token)), "x", "x", owner, feeRecipient, CAP, MIN, DELAY, 599, 50, 500
        );
    ...
    function test_constructorRevertsStalenessTooHigh() public {
        vm.expectRevert(abi.encodeWithSelector(MeiVault.StalenessOutOfRange.selector, 1 days + 1));
        new MeiVault(
            IERC20(address(token)),
            "x",
            "x",
    ...
    function test_constructor_revertsOnBadWithdrawalDelay() public {
        vm.expectRevert(abi.encodeWithSelector(MeiVault.DelayOutOfRange.selector, 3599));
        new MeiVault(
            IERC20(address(token)), "x", "x", owner, feeRecipient, CAP, MIN, 3599, STALE, 50, 500
        );
    }

    function test_constructorRevertsDelayTooLow() public {
        vm.expectRevert(abi.encodeWithSelector(MeiVault.DelayOutOfRange.selector, 1 hours - 1));
        new MeiVault(
            IERC20(address(token)),
            "x",
            "x",
    ...
    function test_constructorRevertsDelayTooHigh() public {
        vm.expectRevert(abi.encodeWithSelector(MeiVault.DelayOutOfRange.selector, 30 days + 1));
        new MeiVault(
            IERC20(address(token)),
            "x",
            "x",
    ...
    function test_execOnlyExecutor() public {
        ...
        vm.prank(user);
        vm.expectRevert(MeiVault.NotExecutor.selector);
        vault.exec(address(target), abi.encodeWithSelector(target.ping.selector, 1));
    }
    ...
    function test_execBubblesTargetRevert() public {
        ...
        vault.exec(address(target), abi.encodeWithSelector(target.reverts.selector));
    }
    ...
    function test_execEmitsExecuted() public {
        ...
        vault.exec(address(target), data);
    }
    ...
    function test_execNonReentrant() public {
        ...
        vm.prank(executor);
        vm.expectRevert();
        vault.exec(address(reentrant), abi.encodeWithSelector(reentrant.attack.selector));
    }

    function test_execDropsValueParam() public {
        vm.prank(owner);
        vault.setAllowedTarget(address(target), true);
        vm.prank(executor);
        bytes memory ret =
            vault.exec(address(target), abi.encodeWithSelector(target.ping.selector, 9));
        assertEq(abi.decode(ret, (uint256)), 10);
        assertEq(address(target).balance, 0);
        assertEq(target.lastValue(), 0);
    }

    function test_oldExecValueSignatureIsAbsent() public {
        vm.prank(owner);
        vault.setAllowedTarget(address(target), true);
        ...
        (bool ok,) = address(vault)
            .call(
                abi.encodeWithSignature(
                    ...
                    address(target),
                    0,
                    abi.encodeWithSelector(target.ping.selector, 9)
                )
            );
    ...
    function test_execCannotForceValueViaForceFundedEth() public {
        ForceEth forceEth = new ForceEth{value: 1 ether}();
        forceEth.destroy(payable(address(vault)));
    ...
    function test_execCanCallMultipleTargetsSequentially() public {
        ...
        vault.exec(address(target), abi.encodeWithSelector(target.ping.selector, 1));
        vm.prank(executor);
        vault.exec(address(target2), abi.encodeWithSelector(target2.ping.selector, 2));
    ...
    function test_feeConstructorBounds() public view {
    ...
    function test_receiveRevertsWithNamedError() public {
        vm.deal(user, 1 ether);
        vm.prank(user);
        (bool ok, bytes memory ret) = address(vault).call{value: 1 ether}("");
        assertFalse(ok);
        assertEq(ret, abi.encodeWithSelector(MeiVault.EtherNotAccepted.selector));
    }

    function test_executedEventDoesNotIncludeValue() public {
        vm.prank(owner);
        vault.setAllowedTarget(address(target), true);
        bytes memory data = abi.encodeWithSelector(target.ping.selector, 7);
    ...
    function test_cancelRedemptionAfterUnlockSucceeds() public {
        uint256 shares = _depositAs(user, 100e6);
        uint256 id = _requestAs(user, shares, user, user);
        vm.warp(block.timestamp + DELAY);
    ...
    function test_decimalsOffsetCorrectForUSDClikeAsset() public {
        MockERC20 usdc = new MockERC20("USD Coin", "USDC", 6);
        vm.prank(owner);
        MeiVault usdcVault = new MeiVault(
            IERC20(address(usdc)),
            "USDC Vault",
            "vUSDC",
            owner,
        ...
        vm.startPrank(user);
        usdc.approve(address(usdcVault), 100e6);
        usdcVault.deposit(100e6, user);
        vm.stopPrank();

        assertEq(usdcVault.balanceOf(user), 100e18);
        assertEq(usdcVault.decimals(), 18);
