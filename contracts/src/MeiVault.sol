...
contract MeiVault is ERC20, Ownable2Step, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        if (valuationStalenessSeconds_ < 600 || valuationStalenessSeconds_ > 1 days) {
            revert StalenessOutOfRange(valuationStalenessSeconds_);
        }
        if (withdrawalDelaySeconds_ < 1 hours || withdrawalDelaySeconds_ > 30 days) {
            revert DelayOutOfRange(withdrawalDelaySeconds_);
        }

        uint8 assetDecimals = IERC20Metadata(address(asset_)).decimals();
        // See spec §10.1 — decimals offset normalizes 6-decimal assets like USDC into
        // 18-decimal shares so one human asset unit mints one human share at inception.
