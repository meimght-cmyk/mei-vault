...
- **Phase 4 — vault contract + signer + audit:** **deployed to Base Sepolia 2026-05-21** at [`0x5328f7c9b6CE55d1f25c20d903F44d33E8F9B5e6`](https://sepolia.basescan.org/address/0x5328f7c9b6CE55d1f25c20d903F44d33E8F9B5e6). 3,360 bytes of bytecode; deploy tx in [docs/deployments.md](docs/deployments.md). Initial state: deployer = owner = Kewe operator wallet (collapsed roles on testnet); no executor set yet; no allowlist entries yet.
  - [`contracts/TreasuryVault.sol`](contracts/src/TreasuryVault.sol) — Ownable2Step + Pausable + ReentrancyGuard, single executor with target allowlist, owner-funded treasury model. 26 Foundry tests passing.
  - [`scripts/lib/signer.ts`](scripts/lib/signer.ts) — bounded-delegation signer library. Dry-run by default, refuses Base mainnet without `allowMainnet`, enforces riskclaw attestation freshness, source-specific policy gates. 20 unit tests passing. Operator CLI at [`scripts/signer-cli.ts`](scripts/signer-cli.ts); docs at [`docs/signer.md`](docs/signer.md).
  - Harness + exiter [emit SignRequest JSON files](scripts/lib/signer.ts) when `MEI_VAULT_ADDRESS` is set. Operator processes them via `signer-cli sign`.
  - Mainnet still gated on external audit + 90-day data floor + owner migration to multisig + key separation.
